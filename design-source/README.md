...
**Why every row above says `wireframe`.** The two `/p` files say so in their own
...
reading of what exists today, and it is deliberately not softened: the repo has no
visual-language source yet, and the gate is what makes that state visible instead
of letting the next session infer a look from grey boxes. The visual language of
`/p/*` is settled in #360 (the standard-system row above is the shape it lands
in); until then a UI PR on those surfaces records the owner decision explicitly
