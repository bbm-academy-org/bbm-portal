...
| `finance/RequestBoard.dc.html`                                | `/p/finance/requests` approver board — the picked F2 layout: kanban over the status machine + details slide-over  | `src/app/(platform)/p/finance/requests/**`                    | wireframe | **original** — authored by the Claude lead session for the #339 Stage-A gate from the owner's picked option **D** (kanban over the status machine + details sheet; drag initiates the act, never flips status by itself); pick by Антон, 2026-08-27, recorded on #339. `finance/RequestForm.dc.html` and `finance/RequestQueue.dc.html` are its layout evidence                                                                                                                                                                                                            | #339     |
...
`finance/RequestBoard.dc.html` is authored in that same idiom. That is the honest
