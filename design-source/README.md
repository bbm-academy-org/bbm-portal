...
| `system: shadcn/ui via ui.refine.dev @ default neutral theme + the form / data-table / feedback blocks` | The member cabinet family — the members register, the member record (show/edit) and the create screen — as the REFERENCE surface the other cabinet families are ported against | `src/app/(platform)/p/admin/member/members/**` | visual    | **original** — the same standard system as the first row, now naming the BLOCKS the family renders through and not only the palette: shadcn `form` (react-hook-form + zod), `dialog` / `alert-dialog`, `sonner`, and the Refine `views` + `data-table` + `notification-provider` items of `ui.refine.dev`, all vendored into `src/ui` on #434 (shadcn CLI `4.19.0`, preset `nova`, `@refinedev/react-table` `6.0.1`, `@tanstack/react-table` `8.21.3`, `react-hook-form` `7.87.0`, `sonner` `2.0.8`). Nothing is vendored here — the system's published components ARE the source. Owner Stage-A decision by Антон, 2026-08-26 on #359/#360 (a standard design system at `fidelity: visual` is a legitimate source), applied to this family by the owner's stage-2 «go» on #434, Антон, 2026-09-03. The row is deliberately MORE SPECIFIC than the `p-admin-members.md` wireframe row below, which keeps recording the LAYOUT (Option A: profile left, aliases right) and no visual language. **Design-fidelity: GO — Anton Sidorov, 2026-09-03** | #434     |
| `p-admin-members.md`                                                                                    | `/p/admin` → Участники list plus member create/edit/detail and nested aliases                                                                                                  | `src/app/(platform)/p/admin/member/**`         | wireframe | **original** — verbatim described-layout pick from owner Антон, 2026-08-29, recorded on #316: option A, profile left + aliases right, stacking on narrow viewports. The list layout remains `p-admin-shell.html`; the look remains the accepted `system: shadcn/ui via ui.refine.dev @ default neutral theme` row above. **Design-fidelity: GO — Anton Sidorov, 2026-08-29**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | #316     |
| `p-admin-hours.md`                                                                                      | `/p/admin` → Часы: periods, rates and grades, and Mattermost publication                                                                                                       | `src/app/(platform)/p/admin/hours/**`          | wireframe | **original** — verbatim described-layout pick from owner Anton Sidorov, 2026-08-30, recorded on #317: option A, three separate calm resource/action pages after the owner rejected the temporary-app JSON export during acceptance. The shell remains `p-admin-shell.html`; the look remains the accepted `system: shadcn/ui via ui.refine.dev @ default neutral theme` row above. **Design-fidelity: GO — Anton Sidorov, 2026-08-30**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | #317     |
...
| `finance/References.dc.html`                                                                            | `/p/admin` → Финансы reference tables (счета, проекты, продукты, назначения, статьи, валюты)                                                                                   | `src/app/(platform)/p/admin/finance/**`        | wireframe | **original** — same canvas, same validation and extraction as `finance/Main.dc.html`. The wireframe records the LAYOUT only; the look remains the accepted `system: shadcn/ui via ui.refine.dev @ default neutral theme` row above. **Design-fidelity: GO — Anton Sidorov, 2026-08-31**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | #338     |
| `finance/Overview.dc.html`                                                                              | `/p/finance` overview; F1 builds only its cash-balances card, the rest is F3 (#340)                                                                                            | `src/app/(platform)/p/finance/**`              | wireframe | **original** — same canvas, same validation and extraction as `finance/Main.dc.html`. The wireframe records the LAYOUT only; the look remains the accepted `system: shadcn/ui via ui.refine.dev @ default neutral theme` row above. **Design-fidelity: GO — Anton Sidorov, 2026-08-31**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | #338     |
| `finance/RequestForm.dc.html`                                                                           | `/p/finance/requests` → «Новая заявка» member form (F2 intake); layout evidence for the details sheet                                                                          | `src/app/(platform)/p/finance/requests/**`     | wireframe | **original** — artboard of the same canvas «Финконтур BBM — вайрфреймы» (artifact `ead41905-c726-42b8-bcdb-4f79b80aab09`), authored by the Claude lead session for #115 discovery; data-model validation by Антон, 2026-08-25, recorded on #115; bytes extracted verbatim from the artifact's `appifact-doc` block on 2026-08-27; layout evidence for the F2 surfaces, Stage-A pick recorded on #339 (2026-08-27) **Design-fidelity: GO — Антон, 2026-09-03** — the stock shadcn/Refine theme (the #434 block set) is the visual source for `src/app/(platform)/p/finance/requests/**`; this row keeps recording the LAYOUT only.                                                                                                                                                                                                                                                                                                                                                                                                                 | #339     |
| `finance/RequestQueue.dc.html`                                                                          | `/p/finance/requests` approver queue (F2 intake); layout evidence for the card and the approve/refuse acts                                                                     | `src/app/(platform)/p/finance/requests/**`     | wireframe | **original** — same canvas, same authorship, validation and 2026-08-27 verbatim extraction as `finance/RequestForm.dc.html`; layout evidence for the F2 surfaces, Stage-A pick recorded on #339 (2026-08-27). **Predates spec v2 — where the two disagree, the spec wins** (bytes stay verbatim): its «отклонение следа в леджере не оставляет» is overruled by EARS-512 (a refused request stays visible with its reason and documents), and its «Одобрить → провести» single act is refined by EARS-511/531 (approval without a confirming document authorizes only; posting is the later one-act confirmation) **Design-fidelity: GO — Антон, 2026-09-03** — the stock shadcn/Refine theme (the #434 block set) is the visual source for `src/app/(platform)/p/finance/requests/**`; this row keeps recording the LAYOUT only.                                                                                                                                                                                                                 | #339     |
| `finance/RequestBoard.html`                                                                             | `/p/finance/requests` approver board — the picked F2 layout: kanban over the status machine + details slide-over                                                               | `src/app/(platform)/p/finance/requests/**`     | wireframe | **original** — static-HTML wireframe authored by the Claude lead session for the #339 Stage-A gate from the owner's picked option **D** (kanban over the status machine + details sheet; drag initiates the act, never flips status by itself); pick by Антон, 2026-08-27, recorded on #339. Not a canvas artboard — hence `.html`, not `.dc.html`; `finance/RequestForm.dc.html` and `finance/RequestQueue.dc.html` are its layout evidence **Design-fidelity: GO — Антон, 2026-09-03** — the stock shadcn/Refine theme (the #434 block set) is the visual source for `src/app/(platform)/p/finance/requests/**`; this row keeps recording the LAYOUT only.                                                                                                                                                                                                                                                                                                                                                                                      | #339     |
| `system: shadcn/ui via ui.refine.dev @ default neutral theme + the data-table / form / feedback blocks` | `/p/finance/requests` — the REQUESTS TABLE that is now the route's default view for every signed-in member (date · submitter · amount · purpose · status · refusal reason, «мои / все» filter) | `src/app/(platform)/p/finance/requests/**` | visual | **original** — the standard system's own published theme and block set; nothing is vendored, and there is no mockup for this view because the owner adopted the system instead. Owner **decision 35**, Антон, 2026-09-14, issue [#115](https://github.com/bbm-academy-org/bbm-portal/issues/115) (brainstorm comments of that day): the route stays one page, the default view for everyone is a table of all requests, and the picked kanban (`finance/RequestBoard.html` below) becomes a board toggle for `finance-approve`. Same system as the first row of this table — shadcn CLI `4.19.0`, preset `nova`, base `radix`, registry `ui.refine.dev` + `ui.shadcn.com`, Tailwind `4.3.3` — with the `table`, `tabs`, `badge`, `button` and `form` items the view renders through. The view is rendered THROUGH the Refine `data-table` BLOCK — `@/ui/refine-ui/data-table/data-table` + `data-table-pagination` driven by `useTable`, the row `docs/design/ui-whitelist.md` settles for «a register of records with paging» — and the screen owns only its `ColumnDef[]`. The earlier «the block wants a paged resource and this surface reads one snapshot through `useCustom`» justification is **WITHDRAWN** (owner acceptance of PR #470, Антон, 2026-09-15): TanStack accepts any array, and the board's own data provider answers `getList` from that same snapshot. The block itself gained a totals row on the same issue, rendered from TanStack's `getFooterGroups()` through the kit's `TableFooter` (`src/ui/README.md` → the divergence table). **Design-fidelity: GO — Антон, 2026-09-14 — decision 35 adopts the standard system for this view** | #388 |

**Why every FILE row above says `wireframe`.** The two `/p` files say so in their own
provenance («static-HTML wireframe»), the five finance artboards come from a
canvas whose own title is «Финконтур BBM — **вайрфреймы**», and
`finance/RequestBoard.html` says so in its own provenance too. That is the honest
reading of what exists today, and it is deliberately not softened: none of those
eight files carries a visual language, and the gate is what makes that state
