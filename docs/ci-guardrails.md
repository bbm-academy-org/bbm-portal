...
| _instruction-budget_    | _CLI only_           | _the always-on agent context (CLAUDE.md + AGENTS.md + `.claude/rules/*.md` + MEMORY.md) over 200 lines / 25 KB per file — landed in task 7.8 (#139) as a CLI guard, §6.1_                  | _n/a — §6.1_ | 2026-08-05 | **CLI-only; `ci.yml` wiring deferred** — no task in epic 7 wires it (7.4 shipped the measurement, not the job). Wiring it is what starts a §4 clock; until then §6.1's exit 1 is the severity of record |
