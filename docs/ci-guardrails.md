...
| **tdd-order**           | `ci.yml`             | a NEW production file under `src/lib/**` or `src/app/(platform)/p/**` whose test is first cited by the SAME commit that introduced it, or by a LATER one — the ORDER half of the TDD contract `test-presence` does not read. A module no commit tests at all is `test-presence`s finding and is silent here. Modified-only files are out of scope in v1 (named blind spot in the guard header)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **BLOCK** | 2026-08-27 | day-0 BLOCK under §3 class 1 — the input is the PR commit graph (which commit introduced which file, which commit adds the citing line), so the false-positive class is empty by construction and a WARN soak would only re-prove what reading the guard proves. Mandate recorded by #355 after PR #354 passed `tdd-signal` green with an implementation written before its tests. Blast radius: a PR that squashed test and implementation into one commit; the fix is an interactive rebase splitting that commit, run by whoever opened the PR                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
promotion 2026-09-02); `tdd-order` starts at BLOCK and has no clock to run.
