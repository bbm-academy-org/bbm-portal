...
where one such flag would mask every guard step inside it at once. The step-level half of
the same invariant — a WARN guard step whose finding nothing surfaces — is the
`unaggregated-warn-step` class in the §5 row.
...
| **workflow-auth**       | `ci.yml`             | a workflow job that reaches GitHub through `gh` without `permissions:` + `GH_TOKEN`/`PR_NUMBER` wiring; **and the severity classes of §2.1** — `undeclared-severity` (a `ci.yml` job that is neither WARN nor BLOCK), `vacuous-block` (needs-listed AND `continue-on-error: true`), `unaggregated-warn-step` (a guard step with step-level `continue-on-error: true` and no row reading `steps.<id>.outcome` in its job's WARN-outcomes step, an `id:`-less such step included) | WARN      | 2026-08-05 | earliest 2026-09-02, §4 clauses. The two severity classes landed in #207; all four classes share the one exit code and therefore the one severity (§2)                                        |
