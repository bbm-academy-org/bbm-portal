...
- **Moving them into `ci.yml` was rejected**, for the reason `pr-body-guards.yml`'s own
  ...
  treats a skipped need as red, so a body-only edit would poison it. That decision stands.
- **`pnpm pr:land` did not need teaching for the FAILURE direction.** `classifyChecks`
  (`tools/gh/pr-land.mjs`) demotes a FAILURE check-run to a printed warning **only** for
  names it resolved into the WARN plane from the base ref's workflow files (#397). A job
  that carries no `continue-on-error` is not in that plane, so its FAILURE lands in
  `failed` and the gate's verdict is `red`. Dropping the flag IS the promotion.
- **The `if:` fence had to go with it.** `classifyChecks` passes a `SKIPPED` conclusion, and
...
context list is `["ci"]` and adding a cross-workflow job to it would wedge merges the day
that job legitimately skips. A merge performed by hand in the GitHub UI bypasses these
five, exactly as it bypasses every other non-`ci` mechanism here. Stated so nobody has to
discover it.
...
| **test-presence**       | `ci.yml`              | a PR that changes production source and ships no test, in a module that has no test either — PRESENCE only; it was called `tdd-signal` until 2026-08-27 (#355) and the name claimed an attestation it never made                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438). Its one recorded defect is a false PASS, not a false BLOCK — the substring path match lets a spec that merely MENTIONS a module path count as covering it (DEBT.md) — so it can miss a violation but cannot invent one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **tdd-order**           | `ci.yml` + pre-commit | **two enforcement points, one rule, two scopes.** PR plane (`ci.yml`): a NEW production file under `src/lib/**` or `src/app/(platform)/p/**` whose test is first cited by the SAME commit that introduced it, or by a LATER one. PRE-COMMIT plane (`--staged`, a second command on the existing `simple-git-hooks` line): blocks **MIXED commits only** — a new module staged together with a staged test citing it, the #354 shape and the #355 retro mandates literal wording. A new module NO test cites is a printed ADVISORY at exit 0 there, never a rejection. Modified-only files are out of scope in v1 on both                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **pre-commit BLOCK / CI WARN** | 2026-08-27 | **Pre-commit half: BLOCK from day 1 under §3 class 1** — local tree and index through git plumbing (no network, no PR metadata) AND a blocking rule whose false-BLOCK class is empty by construction, because a miss of the `needlesFor` matcher on the MIXED direction lets a commit through rather than stopping one. That second half is load-bearing and was got wrong first: round-2 review of PR #394 measured the earlier «reject a module no test cites» rule at **41 of 78 platform files on `main` (53%)** false-BLOCKed — barrels are structurally uncitable, route layouts and Drizzle tables are not imported by path — and a gate whose correct-usage answer is `--no-verify` is the dead end §3 clause 3(d) forbids. Narrowing to MIXED-only cleared it; the uncited case is now an advisory. **CI half: WARN v1 by the §3 default**, settled 2026-08-27 (class 1 excludes it: PR metadata over the network, substring heuristic; no owner class-3 mandate sought). Promotion per §4, earliest **2026-09-24**. Remaining CI-plane false-positive class, enumerated in the guard header: merge-commit first-parent diffs and truncated commit file lists closed in code; substring needles remain, as a false PASS. **Soak evidence: one CONFIRMED FALSE BLOCK on day 0** (2026-08-27, PR #396) — a tests-only commit cited the new module through its public barrel (`@/lib/finance`, what ADR-002 prescribes for a test of the public API) and the literal-path needle did not see it, so the verdict flipped to `impl-first` on an honest branch. Class closed by #398 / PR #400 (`reexportEdges` resolves the barrels re-exporting a NEW module and accepts a citation of those paths, for that module only); the widening is monotone, so it can only false-PASS. The 2026-09-24 promotion decision reads this row: one confirmed false BLOCK, fixed, soak continues |
| **endpoint-authz**      | `ci.yml`              | an exported HTTP method under `src/app/**/api/p/**/route.ts` that is neither built by `memberRoute`/`adminRoute` (`src/lib/platform/api`) nor gated by `claimGateResponse` in its own file; a handler under `/api/p/<slug>/admin/*` built with `memberRoute`; and the reserved segment `admin` sitting anywhere but directly after the module slug (spec 311 EARS-461/EARS-462, D-12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **BLOCK**                      | 2026-08-27 | day-0 BLOCK under §3 classes **1 and 2** together. Class 1: the only input is the checked-out tree — no network, no PR metadata, and the patterns matched are this repo’s OWN handler convention declared in `src/lib/platform/api/moduleRoute.ts`, not human prose. Class 2: the written decision that the caught failure is a data-protection incident is spec 311 EARS-461/EARS-462 («a handler that relies on the shell having checked is a defect») and consolidation §5 («authorization lives in every handler; UI-level restriction is convenience, never a trust boundary») — a WARN soak over an authorization hole is itself the risk. It is also the deferred §11 row of the consolidation spec («endpoint-authz-аналог для route handlers»), landed with the surface that triggers it (#315). Escape hatch, §3 clause (d) on the CI plane: `// endpoint-authz-ok: <reason>` on the handler line — reachable without leaving the PR, spent on that one handler, and read where the decision was made. Demotion follows §4 on the first confirmed false block                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **no-stub**             | `ci.yml`              | a user-facing dev placeholder ("set this env var"), or a `TODO`/`FIXME`/`STUB` standing in for a deliverable with no `#NNN`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438): clean window, no recorded defect in either direction. #438's own body listed 11 guards and omitted this one; this register, not the issue text, is the specification, so it was promoted with the rest                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **workflow-auth**       | `ci.yml`              | a workflow job that reaches GitHub through `gh` without `permissions:` + `GH_TOKEN`/`PR_NUMBER` wiring; **and the severity classes of §2.1** — `undeclared-severity` (a `ci.yml` job that is neither WARN nor BLOCK), `vacuous-block` (needs-listed AND `continue-on-error: true`), `unaggregated-warn-step` (a WARN guard step in a batch job whose `steps.<id>.outcome` no other step of that job reads, an `id:`-less such step included — the obligation itself lives in §8, «Wiring convention for a batch job»); an expression-valued `continue-on-error:` is `undeclared-severity` too — it declares no severity at all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438) — all three recorded defects are false PASSES (a guard reached through a composite `uses:` action, a matrix-interpolated invocation, and no check that a workflow declares a top-level `permissions:` floor at all), so blocking on what it does catch cannot stop a legitimate PR. The two severity classes landed in #207; all four classes share the one exit code and therefore the one severity (§2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **epic-autoclose**      | `pr-body-guards.yml`  | `Closes #<epic>` on a PR whose target issue still has OPEN sub-issues — merging would auto-close a live epic                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438): clean window, no recorded defect, and the rule reads the native sub-issue graph rather than prose. The cross-workflow decision §2.1 demanded was taken with it and is recorded there                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **assignee-milestone**  | `pr-body-guards.yml`  | an open PR with no assignee or no milestone — an un-triageable board row                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438). Its DEBT.md line (#80 — agent-opened PRs arriving with neither field) records a TRUE positive, which §4 clause 3 counts FOR promotion, and it has not fired since the 2026-08-15 sweep. Blast radius: a finding is unblocked with one `gh pr edit --add-assignee … --milestone …`, and since #449 the workflow listens on `assigned`/`unassigned`/`milestoned`/`demilestoned`, so that edit re-runs the check itself instead of leaving the FAILURE standing until the next push. §7 says why it was not a day-0 BLOCK here                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **spec-link**           | `pr-body-guards.yml`  | a feature PR (a `Feature` issue or a `feat:` title, changing `src/`) that resolves to no spec — or resolves to one that is missing, statusless, or still `Draft`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438). Guard authored in task 7.4 (#135) and always invoked with `--severity block`, so the real signal §4 clause 1 demands was there throughout the soak. It is the one promoted guard carrying a live false-BLOCK class — a chore slice of a `Feature`-typed epic can inherit the parent's type through `Part of #N` (DEBT.md, #299/PR #303 N3) — and it is promotable anyway because §4 clause 3 is literally satisfied: the class has never fired, so it is IDENTIFIED, not CONFIRMED. A §3(d)-shaped escape — `spec-exempt: <reason>`, reachable without leaving the PR — bounds it if it does; §3 is the day-0 posture, not a §4 promotion clause. Demotion per §4 on the first confirmed false block                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **instruction-budget**  | `ci.yml`              | the always-on agent context (CLAUDE.md + AGENTS.md + `.claude/rules/*.md` + the MEMORY.md index) over 200 lines / 25 KB per file, **or the corpus sum over 800 lines / 100 KB**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438), which ends the two-plane asymmetry the note below this table described: the same script has been BLOCK on its CLI plane since day 0 under §3 class 1, and its false-positive class is empty by construction. Guard landed in task 7.8 (#139) CLI-only; **wired into `ci.yml` by #157**, which is what started this plane's clock. No `--severity` flag — see §6.1 for why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **stage-b**             | `pr-body-guards.yml`  | a UI diff (non-test `*.tsx` / `*.css` under `src/`) with no recorded `Stage-B:` verdict in the PR body or a linked-issue comment; and, since #485, a recorded `Stage-B: GO` on such a diff with no `UX-sanity:` record — a verdict, the screenshots judged, and a verdict per facet of task-cycle stage 5 item 4 — in the PR body, one of the PR's OWN comments, or a linked-issue comment. Only the `GO` carries that sibling record: `batched at #<gate>` defers the acceptance and `N/A … lead-certified` says there is no surface, so neither has an invitation to a live stand in front of it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438). Guard authored in task 7.7 (#138) and always invoked with `--severity block`, so §4 clause 1 was satisfied throughout the soak. All three recorded defects are false PASSES — an asset-only visual change is invisible to the path-only classifier (#230), a parent-comment GO satisfies every later slice (#299), and the GO/PLACEHOLDER ordering could accept an unfilled template (#359) — so it can miss a UI diff but cannot invent one. **Scope extended 2026-09-15 (#485)** with the `UX-sanity:` sub-check, at the row's existing BLOCK severity: the lead's UX-sanity pass (owner rule, 2026-08-31) left no artifact, so nothing told a lead that ran it from one that skipped it — and on 2026-09-15 the owner became the first reader of PR #470's screen and rejected the stand. Unblocked from inside the session by posting the record as a PR comment; the workflow's `edited` trigger re-runs the check without a rebuild                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **ux-record**           | `pr-body-guards.yml`  | a UI diff (the `stage-b` definition, imported from it) whose PR body — or a linked-issue comment — carries no `UX-record:` block, or one that leaves any of the six facets (composition, controls, grouping, states, feedback, post-submit) unrecorded or still at the PR template's placeholder. The one marker-level escape is the lead self-certification `UX-record: N/A (no UX decisions) — lead-certified`, symmetric with `stage-b`'s; a bare `N/A` is not evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | WARN                           | 2026-09-02 | earliest 2026-09-30, §4 clauses. Guard authored in #433 from the owner ruling of 2026-09-02 (composition and the rest are the agent's decision, recorded in the PR); invoked with `--severity block` so the script gives a real signal while the CI plane stays WARN. Blast radius when promoted: every UI PR, unblocked by adding the block to the PR body — one edit, no re-run of the build matrix (this workflow re-runs on `edited`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **primitives-first**    | `pr-body-guards.yml`  | on the ADDED lines of a PR's diff, in non-test `*.tsx` under `src/app/(platform)`: a raw `<button>` / `<table>` / `<select>` / `<input>` while the kit owns the equivalent (`src/ui/<tag>.tsx` exists), and `useState` beside a `<form>` the diff builds WITHOUT the kit `Form` block (one finding per file, not per hook). A raw `<form>` is deliberately NOT a finding: `src/ui/form.tsx` is a context provider that renders no element, so the documented shadcn shape keeps `<form onSubmit={form.handleSubmit(…)}>` inside `<Form {...form}>` and the kit has no `<form>` equivalent to compose (review of PR #459). The inline escape is `primitives-first-ok: <reason>` on the tag's line or within 5 lines above it — the reason is required, and the narrow window keeps one exception from silencing the next defect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | WARN                           | 2026-09-03 | earliest 2026-10-01, §4 clauses. Ported from ds-platform `tools/lint/primitives-first-lint.ts` in #435, from the owner ruling of 2026-09-02 («the ready libraries are sufficient, nothing is to be reinvented»). WARN is deliberate rather than the §3 default: the existing corpus under `src/app/(platform)` is full of these violations, and a BLOCK guard landing on a red corpus gets routed around rather than obeyed (#435 «Notes»). The §4 clock therefore also needs the per-screen migration tasks to have drained the backlog — promotion is not a date alone                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **interaction-states**  | `pr-body-guards.yml`  | the states half of the same diff: an added opening tag carrying `onClick` under `src/app/(platform)` with no hover affordance, no `focus-visible:` ring, or — when it carries a `disabled` attribute — no `disabled:` treatment. Since #483 a CAPITALISED tag is judged too, on the same rule, when it is a KIT component (exported by `src/ui/*.tsx` in the tree, or imported from `@/ui/…` in the diff) that the diff does not shadow with a non-kit import or a local declaration, carries no `asChild` (the CHILD would own the states) and is not one of `STATE_OWNING_KIT_CONTROLS` — the enumerated kit controls whose `onClick` is a pass-through to a control that already ships hover/selection, focus and disabled (`Button`, `DropdownMenuItem`, `SelectItem`, …; each row cites the kit line that carries the treatment). The judgement is of the CALL SITE. An app-local or third-party component is still skipped: its states cannot be classified from the diff. Inline escape: `interaction-states-ok: <reason>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | WARN                           | 2026-09-03 | **earliest 2026-10-13** — §4 clause 2 restarted the clock on 2026-09-15, when #483 widened WHAT THIS GUARD MATCHES (capitalised kit tags entered scope); a message rewording would not have moved it, a change to the matching does. Same restart the `ears-naming` row below records for its own narrowing, and the same 28 days. §4 clauses otherwise unchanged; same WARN reasoning as `primitives-first` above. Ported from ds-platform `tools/lint/interaction-states-lint.ts` in #435, whose measurement is the motive: `onClick` 27 times against 6 `hover:` occurrences under `src/app/(platform)`, and `cursor-pointer` zero times in `src`. The original's layer-1 (`globals.css` base reset) and `interactiveBase`-fragment halves are DROPPED, not ported — this repo's kit is stock shadcn/ui via Refine (#360/#434) and has neither, so porting them would invent a contract nobody agreed to. WIDENED by #483 (2026-09-15) to clickable KIT components, severity unchanged and the workflow untouched: the lowercase-only rule let PR #470's `<TableRow onClick=… className="cursor-pointer">` through, and the owner reported that defect class by hand on the live stand (#481, «why round 1 was not enough», item 2). Re-run against PR #470's diff, the widened guard reports `RequestsTable.tsx:96`, and nothing else in that diff                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **whitelist-blocks**    | `pr-body-guards.yml`  | on the ADDED lines of ONE PR's diff, in non-test `*.tsx` under `src/` outside `src/ui`: a file that BUILDS an element class `docs/design/ui-whitelist.md` has SETTLED (Form / List / Feedback) without importing that row's implementation — an added import of a kit FIELD primitive (`@/ui/input`/`select`/`label`/…) in a diff that also opens a `<form>`, with no `@/ui/form` composition; an `@/ui/table` import (or a raw `<table>`) with no Refine `data-table` block; a browser `alert()` announcing an outcome instead of the sonner/Refine toast. Every signal is an added IMPORT, never a bare JSX tag (round-1 review of PR #489 — a raw `<form>` fired on the fieldless sign-out and GET-filter forms that live on `main`, and `primitives-first`'s row above already settled that the kit has no `<form>` equivalent). Two widenings this row OWNS rather than denies: `confirm()` is deliberately NOT a finding (a prompt is not an outcome; that is the unsettled `dialog`/`AlertDialog` class), and the List signal reads the class by PRIMITIVE while the registry row says «with paging», so a static summary table on `@/ui/table` is a finding too. Cleared ONLY by an owner record in the PR body or a linked-issue comment — `Bespoke-UI: GO — <owner, date> — <what was approved>`, tail required, HTML comments and fenced examples stripped. A second rule reports DRIFT between the guard's row table and the registry, on a PR that touches the registry; that finding is deliberately not clearable by the record, because it is about the guard rather than about the surface | **BLOCK**                      | 2026-09-15 | day-0 BLOCK under §3 **class 3** (owner-mandated process guard whose WARN form already failed), the `design-fidelity` precedent. The clauses, in order: (a) the owner mandated it at BLOCK from day one — Антон, 2026-09-15, #481's decision «сначала правим дисциплинарные гейты» and #482's own «Severity BLOCK from day one» scope line; (b) the rule exists as prose in `.claude/skills/build-ui-from-design-system/SKILL.md` rung 2 since #434 and has a dated recurrence — PR #470's `RequestsTable.tsx`, 2026-09-15, the hand-built table self-certified in its own doc-comment and the live stand rejected; (c) WARN guards over the same surface were live throughout that recurrence and changed nothing (`primitives-first` sees only a RAW `<table>` tag, `interaction-states` only a lowercase clickable — both green on that PR), so the soak §4 would prescribe is the experiment that already ran; (d) the escape hatch is reachable from inside the blocked PR, demands a reason, names the person and the day, and is recorded where reviewers read it — the `Bespoke-UI: GO` marker. Demotion follows §4 on the first confirmed false block                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **ears-naming**         | `ci.yml`              | a test title that ATTEMPTS the `EARS-N:` prefix and misspells it (`ears-3:`, `EARS3:`, `EARS-3` with no colon, `EARS 3:`) — the traceability grep then silently misses it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | WARN                           | 2026-08-05 | **held at WARN on 2026-09-02** (#438); guard tranche 2 (#157). The §4 window was clean, but the guard's own header declared an unresolved FALSE-BLOCK class and deferred it to exactly that review: `ATTEMPT_RE` fired on any title merely STARTING with the word EARS, so `describe('EARS adoption record')` read as a botched id. That class is **closed** — `ATTEMPT_RE` was narrowed (#447) so the word EARS followed by a space and a letter is prose, while `ears-3:`, `EARS3:`, `EARS-3` with no colon and `EARS 3:` stay findings. Narrowing is a substantive rule change, so §4 clause 2 restarted the clock at the day it landed, 2026-09-03. **Remaining condition: the four-week clean window that clause yields — earliest 2026-10-01** (this PR promotes nothing; it closes the class and resets the date the 2026-09-02 review had guessed forward)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **ears-test**           | `ci.yml`              | an `EARS-N` clause declared in a spec's `## Requirements` that no test title cites, or a test citing a clause no spec declares (orphan), or a stale entry in the deferral allowlist; **also the wrong-tree class** — zero spec FILES scanned is exit 1, fail-closed (a corpus that HAS specs but no clauses yet is the documented on-touch state and exits 0)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | WARN                           | 2026-08-05 | **held at WARN on 2026-09-02** (#438); guard tranche 2 (#157). The window was clean, but #288 is OPEN and is a false-BLOCK class: the EARS id keyspace is flat and global, so ids collide across specs and retired ids are still counted. **New condition: land #288, then a 4-week clean window — earliest 2026-09-30**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **spec-deletion**       | `pr-body-guards.yml`  | a PR deleting a `.md` under `docs/specs` / `docs/superpowers/specs` / `docs/adr` with no sanctioned escape; **and** the repo-wide spec status sweep (statusless, off-ladder, `Superseded` with no live successor); **also the wrong-tree class** — zero spec FILES scanned is exit 1, fail-closed, not a clean sweep                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **BLOCK**                      | 2026-08-05 | **promoted 2026-09-02** (#438): clean window, no recorded defect, and the sanctioned `spec-deletion:` escape in the PR body bounds it (§3 clause d). Guard tranche 2 (#157). Both finding classes are now BLOCK, so the one exit code still carries one severity (§2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
**`ux-record` is the agent half of the design split, and `stage-b` is the owner half.**
...
blocked session and demands the content rather than a waiver, which is the §3 condition a
day-0 BLOCK would still have had to meet.
...
`contents: read` included. Only gh-reaching jobs do — `design-fidelity`, `test-presence` and `tdd-order` in `ci.yml`, and EVERY
job in `pr-body-guards.yml` (a count here would stale on the next job added — #482 found it already had). Before #220 the jobs that declare nothing inherited the repo
...
same script is a §2.3 CLI guard (BLOCK by exit code — §6.1) and a §2.1 CI job. Until
2026-09-02 the two planes carried different severities, and that asymmetry was the whole
point of a WARN soak on a plane where a red X was new; the soak ended and #438 promoted the
CI plane, so the two now agree. The entry stays because one script in two inventories still
has to be readable as one thing — and because §2's rule is that a mix must be RECORDED, not
...
| `LINT_PR_FILES_PAGE_SIZE` | shrink the paged file list's page so a spec can cross a page boundary with two entries      |
...
**The changed-file list is PAGED — never read `--json files` for a verdict.**
`gh pr view <N> --json files` is page-limited (100 files) and says nothing when it
truncates, so a guard deriving its verdict from that array under-reads a large PR and goes
green on a violation it cannot see. **The rule: a guard whose verdict depends on the
changed set calls `ghPrFiles(<n>, cwd)` from `lib/gh.mjs`**, which pages
`repos/{owner}/{repo}/pulls/<n>/files` and returns the same
`{ path, additions, deletions, status }` entry shape. All five consumers were converted in
#449 — `test-presence`, `spec-link`, `stage-b`, `ux-record` and `product-note` (`tdd-order`
reads the commit API instead and has its own 250-commit / 300-file limits, named on
`ghPrCommits` and `ghCommit`). A guard owning its own `gh` runner pages through the shared
`pagePrFiles` loop with its own argv (`ghFilesArgs` in `stage-b` / `ux-record`) rather than
re-implementing the walk. Exhausting the page bound is an ERROR, not a truncated success:
each consumer fails closed, because a guard that read part of the diff has not cleared the
diff. `design-fidelity` was already on the paged endpoint before #449 — it needs each
file's `status` — through its own `gh api … --paginate --slurp` call; it is the precedent,
not an exception.
