...
| _instruction-budget_    | _CLI only_           | _the guard landed in #139 as a CLI guard (§6.1, `tools/lint/instruction-budget-lint.mjs`); the `ci.yml` wiring stays with task 7.4 (#135), which fills this row_                           | _n/a — §6.1_ | —          | row filled at landing                                                                                                                                                           |
...
The two italic rows are guards being written in the parallel branch #135 against the
contract in §8. They deliberately do not touch `ci.yml` / `pr-body-guards.yml` — the
session that lands them wires the job and fills the row, exactly as `stage-b` (#138) was
wired here after its own branch landed: its guard, its spec moved to
`tools/lint/guard-tests/` per §8, its job, its row.
