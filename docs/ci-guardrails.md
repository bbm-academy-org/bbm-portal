...
| **ui-tokens**           | `ci.yml`             | a colour literal in a `src/ui/**` stylesheet other than `src/ui/tokens.css` (a value that escaped the palette derived from `design-source/`); a `var(--bbm-…)` anywhere under `src/**/*.css` naming a token `tokens.css` does not declare (silent at runtime — the declaration is simply dropped); and drift between `src/ui/tokens.ts`, the list the `/p/ui-kit` showcase renders, and what `tokens.css` declares, in either direction. Deliberately NOT a `px` ban, and deliberately scoped to the kit rather than to every stylesheet: EARS-429 keeps the existing `/p/okr` and `/p/hours` bodies unreskinned in this epic, so each surface joins the scope on its own first substantive touch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | WARN      | 2026-08-26 | new-guard posture (§3): it matches regexes over CSS text, so it soaks. Earliest promotion 2026-09-23 under the §4 clauses                                                                                                                                                                                                                                                                                                                |
