...
| **tdd-order**           | `ci.yml`             | a NEW production file under `src/lib/**` or `src/app/(platform)/p/**` whose test is first cited by the SAME commit that introduced it, or by a LATER one — the ORDER half of the TDD contract `test-presence` does not read. A module no commit tests at all is `test-presence`s finding and is silent here. Modified-only files are out of scope in v1 (named blind spot in the guard header)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **BLOCK** | 2026-08-27 | day-0 BLOCK claimed under §3 class 1 — **contested, with the owner**: review of PR #394 found class 1 excludes this guard by name (its only input is PR metadata over the network, matched with a substring heuristic), so absent an owner class-3 record with the approved wording and a reachable escape hatch the §3 default applies and this lands WARN on the §4 clock instead. The false-positive class is NOT empty and the guard header enumerates it: merge-commit first-parent diffs and truncated commit file lists were both real false-BLOCK paths and are closed in code; substring needles remain, as a false PASS. §4 demotion on the first confirmed false positive is the standing safety net either way                                                                                                                                                                                                                                                                                                                                                     |
...
promotion 2026-09-02). `tdd-order` is wired BLOCK **pending the owner's verdict on that
posture** — review of PR #394 found the day-0 mandate was claimed under §3 class 1, which
class 1 excludes by name, and the §3 default for a network-and-heuristic guard is to land
WARN and soak. Its row above states the contest rather than settling it; whichever way the
owner rules, §4's demotion clause applies from the first confirmed false positive.
