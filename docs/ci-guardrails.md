...
| **tdd-order**           | `ci.yml` + pre-commit | **two enforcement points, one rule.** PR plane (`ci.yml`): a NEW production file under `src/lib/**` or `src/app/(platform)/p/**` whose test is first cited by the SAME commit that introduced it, or by a LATER one. PRE-COMMIT plane (`--staged`, a second command on the existing `simple-git-hooks` pre-commit line): the same new-module scope, rejected when no test in HEAD references it — a MIXED commit staging module and test together included. The planes differ in one rule on purpose: a module no test cites at all is `test-presence`s finding in CI and silent there, but rejected at pre-commit, where no test-presence counterpart runs. Modified-only files are out of scope in v1 on both                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **BLOCK** | 2026-08-27 | **pre-commit plane blocks from day 1** and needs no mandate: its only input is the local tree and index through git plumbing — no network, no PR metadata — which is §3 class 1 met on its own terms. Its escape hatch is the pre-existing pair `git commit --no-verify` / `SKIP_SIMPLE_GIT_HOOKS=1`, both of which skip every pre-commit check rather than this one. The **CI plane** severity is the contested one: day-0 BLOCK was claimed under class 1, review of PR #394 found class 1 excludes it by name (PR metadata over the network, substring heuristic), so absent an owner class-3 record with the approved wording the §3 default applies and the JOB lands WARN on the §4 clock. The false-positive class is NOT empty and the guard header enumerates it: merge-commit first-parent diffs and truncated commit file lists were real false-BLOCK paths and are closed in code; substring needles remain, as a false PASS. §4 demotion on the first confirmed false positive is the standing safety net                                                         |
