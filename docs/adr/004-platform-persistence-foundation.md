...
## A2 — every `core` row carries its own audit columns, 2026-09-22

**Context.** §6 gives the platform its table-ownership boundaries and spec 201 (#273) gave it the append-only change journal `core.audit_event`: one row per write, holding the diff, the actor and the door the change came through. That journal is the HISTORY, and it is the only thing this foundation ever said about attribution. What it never answered cheaply is the question every screen asks first — «when and by whom was this row created, and when was it last changed» — because answering it from the journal means scanning it per row, per table, per read.

The absence was not theoretical. As of the 2026-09-22 inventory, of the 21 tables under `src/lib/platform/db/schema/**`, three carried `created_at`, one carried `updated_at`, two carried `created_by` and none carried `updated_by` — each one added ad hoc by the spec that happened to need it, with no shared helper and no rule. The concrete pain that surfaced it: the finance intake item had no submission timestamp at all, so the owner could not see when a request had been filed (#517).

**Decision.** `created_at`, `created_by`, `updated_at`, `updated_by` are **mandatory on every platform table by default**, as a hard rule (owner rule, Антон, 2026-09-22; issue #516). Four sub-decisions make that a mechanism rather than a habit:

1. **One helper, spread into every table.** `auditColumns()` in `src/lib/platform/db/schema/audit-columns.ts` — `timestamptz NOT NULL DEFAULT now()` for the two moments, nullable `integer` for the two actors. A table that genuinely needs `created_by NOT NULL` (`finance_counterparty`, `finance_intake_item`, where the submitter is a required fact) re-declares that one key AFTER the spread rather than making the helper's return type conditional.
2. **The actors are `core.member(id)`, with the FK written by hand in the migration** — `ON DELETE SET NULL` where the column is nullable. A drizzle `.references()` would import `schema/member/` into every other module's schema directory, which is exactly the import §6 keeps out of a module. Same shape `finance_posting.member_id` already carries.
3. **Maintenance is a database TRIGGER, not the write layer.** `core.audit_columns_stamp()` fires `BEFORE INSERT OR UPDATE` on every table. The actor is resolved from the SAME session variable the journal reads — `app.actor_email`, set by `platformTransaction()` — so the row and the ledger cannot disagree about who wrote it; an unmarked connection stamps NULL rather than raising, because refusing a context-less write is already `core.audit_row_change()`'s job. `created_at` / `created_by` are restored from OLD on every UPDATE and are therefore immutable. The rejected alternative was «every caller sets `updated_at`»: a `cli:*` script, a fixture, a seed and a hand-run `psql` statement are all legitimate writers here, and the one that forgets is precisely the one whose row later reads as untouched.
4. **The journal is complementary, not replaced.** The two immutable columns are recorded BY VALUE in the capture trigger's whitelist; the two write-side ones are dropped from the diff entirely, as `updated_at` already was — recording `updated_by` would restate the ledger row's own `actor_email` and would make a touch by a different person write a ledger row for a change that never happened (EARS-3).

**Consequences.**

- Migration `0016_audit_columns.sql` adds the columns, the FKs, the stamping trigger and the re-attached capture triggers, and then **backfills from the journal**: `created_at` / `created_by` from the row's OWN `data.<table>.insert` event — the most recent one under that primary key, because a key is reused after `TRUNCATE … RESTART IDENTITY` and the oldest insert under it can belong to a row that no longer exists — and `updated_at` / `updated_by` from the latest event of any type; a row the journal does not cover keeps `now()` and a NULL actor — the honest «we do not know». The backfill runs with `ALTER TABLE … DISABLE TRIGGER USER`, because left on it would both overwrite what it restores and append one `db-direct` UPDATE event per row to the journal it is reading; `session_replication_role = replica` was rejected as needing superuser, which the migrating role of A1 is not.
- A new guard blocks from day 0: `pnpm lint:audit-columns` (`tools/lint/audit-columns-lint.mjs`), registered in `docs/ci-guardrails.md` §5, with its truth-level counterpart `tests/int/platform/audit-columns.int.spec.ts` in the `platform-int` job.
- Every new `core` table now spreads the helper as a matter of course; the operating instructions live in `src/lib/platform/db/README.md` → «Every row carries audit columns».
- Backfilled `created_at` for the 47 reconstructed finance requests is the RECONSTRUCTION date, not the original filing date. Lifting the real one is #517, on top of this migration.

**Why now.** Owner rule stated on 2026-09-22 in the review of the reconstructed requests, filed as issue #516 — the head of that wave's mechanical path.

**Affects.** §6 (the table-ownership boundaries now have a standing exception shaped as hand-written FKs rather than drizzle references — the mechanism §6 already prescribes, now applied platform-wide) and the «Every write is attributed» contract of `src/lib/platform/db/README.md`, which gains a sibling section. §1–§5 and §7 are untouched.
