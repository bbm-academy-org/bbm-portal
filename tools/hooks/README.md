...
| `worktree-path-guard.mjs`               | PreToolUse `Edit\|Write\|MultiEdit\|Bash\|PowerShell`                            | **BLOCK** записи абсолютным путём из worktree в общий чекаут, включая распознанную обычную PowerShell-запись через `Set-Content`, `Add-Content` или `Out-File`; **WARN** один раз на первую запись в общее дерево при живой параллели                                                                                                                                                                                                                                     | путь внутри ЛЮБОГО worktree (`.claude/worktrees/…` — общим чекаутом не является, #187); shell-вызов без уверенно распознанной цели записи; путь вне репо; нет живых параллельных сессий                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `lead-context-budget.mjs`               | PreToolUse `Agent\|Task`                                                         | **WARN** лиду при ≈150K токенов контекста (`additionalContext`: волну в полёте довести, новую не начинать, довести задачу до точки остановки, финальный отчёт + handoff) и **BLOCK** НОВОГО диспатча при ≈160K (`permissionDecision: "deny"`, #457, порт из ds-platform). Обращён к МОДЕЛИ — в отличие от сиблинга `context-budget.mjs` (тот только к оператору); безопасно, потому что срабатывает ровно в момент запроса НОВОГО диспатча и ничего в полёте не прерывает | вызов ИЗ субагента (`agent_id` в payload) — молчит; маркер `.claude/lead-budget-override` в ОСНОВНОМ чекауте снимает оба порога, но только если его первая строка — причина владельца (пустой маркер не override: пороги действуют, а сообщение это называет), и ГРОМКО — `additionalContext` цитирует причину, stderr пишет её в лог сессии, снимается маркер на /wrap (класс 3 §3, разбор — `docs/ci-guardrails.md` §6); `BBM_HOOKS_DISABLE=1`; нечитаемый/отсутствующий `transcript_path` — молча exit 0                                                                                                                                                                                                                                                                                                                                                                                              |
...
Stop. `request_user_input` and `request_user_input_async` normalize to the
canonical question shape; because async answers have no stable transcript
evidence, the repeated-header ratchet stays disabled for that async call while
the self-contained-question checks remain active. Exact telemetry parity is
not claimed: arbitrary shell reads do not map safely to Claude read tools, and
Codex exposes no stable token count for either context-budget path. Those paths
stay manual rather than pretending to pass.
