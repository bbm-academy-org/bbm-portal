...
- **Невернувшийся фоновый агент снимает гейты** (#415). Диспатч `Agent`/`Task`,
  на `tool_use_id` которого в транскрипте нет ни одного `tool_result`
  ...
  формулировки фан-аута («агенты запущены», «работают в фоне», «жду отчётов»,
  «когда … отчитаются», «background agents»). Блок `tool_use` БЕЗ `id`
