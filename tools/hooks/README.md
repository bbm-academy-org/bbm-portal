...
```sh
BBM_HOOKS_DISABLE=1   # выключает ВЕСЬ стек — уважают все десять хуков
```
...
| `dispatch-guard.mjs`                | PreToolUse `Agent\|Task\|Edit\|Write\|MultiEdit` | **WARN** на 3 правки лида подряд в общем дереве без Agent между ними (task-cycle stage 3); **WARN** on a dispatch brief that stages output to disk instead of applying it (retro 2026-08-05)                                                        | worktree-сессия; `BBM_DISPATCH_GUARD_DISABLE=1`; `Agent` сбрасывает счётчик; an explicit `STAGED: irreversible\|conflicting\|owner-preapproval` token in the brief |
...
| `askuserquestion-context-guard.mjs` | PreToolUse `AskUserQuestion`                     | **BLOCK** a question the owner cannot answer from its own text: a header re-asked without being expanded ≥2×, or a question < 120 chars leaning on a bare `#N` (the owner sees only question+options — prose between tool calls never reaches them) | a header asked for the first time; a rewritten question ≥2× the previous length; a self-contained question ≥120 chars; unreadable `questions[]` payload            |
...
| `deviations-gate.mjs`               | Stop                                             | **BLOCK** остановки, если в отчёте нет строки stage 7 «Отклонения от конвенций:»; also **BLOCK** a self-certified «нет» when the transcript carries an owner halt or an earlier Stop-gate block (retro 2026-08-05)                                  | те же, что у completion-report-gate (распознаватель общий); a report that LISTS deviations always passes, so a corrected report ends the loop                      |
