...
- **Отчёт о завершении требует write-действия** (#158). Все три Stop-гейта ходят
  ...
  `Agent`/`Task`; `Bash`/`PowerShell` по узкому белому списку мутаций;
  `mcp__*`-инструмент с мутирующим глаголом в хвосте имени — точный состав в
  самом файле). Read-only сессия ничего не завершала, отчитываться ей не о чем.
