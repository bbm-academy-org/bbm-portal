...
<!--
2-4 предложения о ВИДИМОМ пользователю изменении, обычным продуктовым языком:
что человек реально заметит, названный продуктовой сущностью (страница сайта,
форма заявки, календарь часов) — без инженерного жаргона.

Это единственный источник правды: при мерже отсюда уходит пост в Mattermost, и
из этих же строк собирается прод-дайджест в момент деплоя. Написать один раз
здесь — и не переписывать больше нигде.

`none` — если изменение внутреннее (chore / CI / рефакторинг / зависимости).
-->
