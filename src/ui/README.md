...
**Seven deliberate divergences from upstream, all of them named:**
...
| the `data-table` block renders a TOTALS ROW                                             | a register that adds its rows up had to hand-build a `<TableFooter>` beside the block — the whitelist violation `lint:whitelist-blocks` catches. TanStack already models it, so the block renders `getFooterGroups()` through the kit's `TableFooter` when any column declares a `footer`, and the screen declares its total as one more `ColumnDef` field (#388, owner go 2026-09-15)                                                                                                                                                               |
| the `data-table` block's EMPTY STATE is pinned to the container, wraps, and is shorter  | upstream shrinks the message to `width: fit-content` and shifts it by `translateX(-50%)` once the table overflows, and the kit's `whitespace-nowrap` `TableCell` stops it wrapping: at 390 px the description ran off BOTH edges of a 341-px container («ы подадите, появится здесь — включая черновики и от»). It is now `sticky; left: 0` at the measured CONTAINER width with both lines `whitespace-normal`, and the cell's hard-coded `height: 490px` — half a phone screen of void — became a class (#388 defect D, eyes-on matrix 2026-09-16) |
