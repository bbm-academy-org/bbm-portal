...
прозы**. Канон: [`.claude/rules/task-canon.md`](../../rules/task-canon.md) §3.
...
pnpm taxonomy:bootstrap --apply  # завести source:*-лейблы и fallback-milestone
```
...
pnpm issue:create --title "<тема набора>" --body-file <файл> \
  --type Task --label epic --label source:spec --milestone "<тема>"
```
...
Каждый ребёнок — через ту же обёртку, `--label source:spec`, тип по классу
...
Заодно проверить секции «Гигиена полей» (Type/source/milestone/assignee) и
...
| Задачи заведены `gh issue create`              | Обошли валидацию: Type/source/milestone могли не проставиться |
